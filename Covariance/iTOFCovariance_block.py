...
def CalculateCovElements(arguments):
    ...
    return NOE,SX,SY,SXI,SYI,SXY,SI,SI2

def CalculateCovMap(arguments):

    fpath=arguments.fpath
    spath=arguments.spath

    NOE,SX,SY,SXI,SYI,SXY,SI,SI2=CalculateCovElements(arguments)
    
    covXY=np.zeros((1000/binning,1000/binning),dtype=np.float64)
    pcovXY=np.zeros((1000/binning,1000/binning),dtype=np.float64)

    N=NOE
    c=1.0
    covXY=(SXY-c*np.outer(SX,SY)/N)/(N-1.0)
    
    covXI=(SXI-(SX*SI)/N)/(N-1.0)
    covYI=(SYI-(SY*SI)/N)/(N-1.0)
    covII=(SI2-(SI**2)/N)/(N-1.0)
    c=1.0
    pcovX=((N-1.0)/(N-2.0))*(covX-c*(np.outer(covXI,covYI)/covII))
    ...
    CovMaps={'covXY':covXY,'pcovXY':pcovXY}
    sio.savemat(fpath+'CovMaps%i_binning%i_x0_%i_y0_%i.mat'%(rank,binning,x0,y0),CovMaps)

CalculateCovMaps(args)
MPI.Finalize()
